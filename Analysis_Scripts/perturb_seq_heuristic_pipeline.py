...
import sys
import os
from plotnine import ggplot, aes, geom_bar, ggtitle, xlab, ylab, scale_fill_manual, geom_histogram, labs, theme, element_text, scale_y_continuous #type: ignore
...
def data_statistics(mdata_rna, mdata_crispr,ntc_delimeter="Non-Targeting"):
    genes = sorted(list(set([gene.split('_')[0] for gene in mdata_crispr.var["gene_ids"]])))
    genes.remove(ntc_delimeter)
    ntcs = [gRNA for gRNA in mdata_crispr.var["gene_ids"] if ntc_delimeter in gRNA]
    avg = (len(mdata_crispr.var["gene_ids"])-len(ntcs))/len(genes)
    print(f"Number of cells detected: {len(mdata_crispr.obs)}")
    print(f"Unique gRNA perturbed genes detected: {len(genes)}\nAverage gRNA detected per gene: {avg:.2f}") #TODO: Investigate this, it seems to be off by 1
    print(f"Total GEX UMIs: {mdata_rna.X.sum():,}")
    print(f"Total CRISPR UMIs: {mdata_crispr.X.sum():,}")
    print(f"Total cells detected: {len(mdata_crispr.obs)}")
    print(f"Total genes detected: {len(mdata_rna.var)}")
    print(f"Total gRNAs detected: {len(mdata_crispr.var)}")
    print(f"Total NTC gRNAs detected: {len(ntcs)}")
    print(f"Average GEX UMIs per cell: {mdata_rna.X.sum()/len(mdata_rna.obs):,.2f}")
    print(f"Average CRISPR UMIs per cell: {mdata_crispr.X.sum()/len(mdata_crispr.obs):,.2f}")


def read_in_10x_mtx_output(mtx_dir, save_filepath,ntc_delimeter="Non-Targeting"):
    ...
    data_statistics(adata_gex,adata_crispr,ntc_delimeter=ntc_delimeter)
    return adata_gex


def plot_gRNA_distribution(adata:ad.AnnData)->None:
    """
    Plot the distribution of the number of gRNAs per cell.
    """
    df = pd.DataFrame(adata.obs['n_gRNA'], columns=['n_gRNA'])

    # Lump all values greater than 6 into a "<6" category
    ...
    plot = (ggplot(df, aes(x='n_gRNA', fill='n_gRNA')) +
            ...
            ylab('Count'))
    plot.show()
    vc = df['n_gRNA'].value_counts()
    no_guide, one_guide, all_guides = vc.loc['0'], vc.loc['1'], sum(vc)
    no_call_rate = (no_guide/all_guides) * 100
    multiplet_rate = ((all_guides - (no_guide + one_guide)) / all_guides)*100
...
def plot_cells_per_guide_distribution(adata:ad.AnnData)->None:
    ...
    # Extract the 'gRNA' column from the AnnData object
    gRNA_series = adata_1_gRNA.obs['gene_target']

    # Get the value counts
    gRNA_counts = gRNA_series.value_counts()
    ...
    # Get the count value for the row with gRNA labeled "NTC"
    ntc_count = gRNA_counts_df.loc[gRNA_counts_df['gene_target'] == 'NTC', 'count'].values[0]

    # Drop the row with gRNA labeled "NTC" from the DataFrame
...
def assign_protospacers(adata, protospacer_calls_file_path):
    protospacer_calls = pd.read_csv(protospacer_calls_file_path,index_col='cell_barcode')
    mask = list(adata.obs_names.isin(protospacer_calls.index))
    adata.obs["gRNA"] = "None"
    adata.obs["n_gRNA"] = 0
    adata.obs["n_gRNA_UMIs"] = "-1"
    adata.obs.loc[mask,"gRNA"] = protospacer_calls.loc[adata.obs_names[mask],"feature_call"]
    reindex = ['-'.join(guide.split('-')[:-1]) + "_" + str(guide.split('-')[-1]) if '|' not in guide else guide  for guide in adata.obs.gRNA]
    adata.obs.gRNA = reindex
    adata.obs["gene_target"] = [guide.split('_')[0] if '|' not in guide else guide for guide in adata.obs.gRNA]
    adata.obs["gene_target"] = ["NTC" if guide == "Non-Targeting" else guide for guide in adata.obs.gene_target]
...
def assign_metadata(adata, cell_type, perturbation_type, subset_to_1_gRNA=True, channel_dict = None, treatment_dict = None):
...
def general_qc(adata):
    qc_adata = adata.copy()
    sc.pl.highest_expr_genes(qc_adata, n_top=20)
    fig, ax = plt.subplots(2,1,figsize=(6,10))
    ax[0].plot((sorted(list(adata.obs["n_genes"]),reverse=True)),'.')
    ax[0].set_xlabel("Cell Number")
    ax[0].set_ylabel("Number of Genes Detected")
    ax[1].plot(np.log1p(sorted(list(qc_adata.var["n_cells"]),reverse=True)),'.')
    ax[1].set_xlabel("Gene Number")
    ax[1].set_ylabel("Log1p Number of Cells Detected")
    plt.show()
    print(f"Removing {sum(qc_adata.var['n_cells'] == 0)} genes not detected in any cells.")
