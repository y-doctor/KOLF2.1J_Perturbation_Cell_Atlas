...
def plot_differentially_expressed_genes(
    results_dict: Dict[str, pd.DataFrame], p_value_threshold: float = 0.05, 
    l2fc_threshold: float = 0, deg_cutoff: int = 25
) -> None:
    """
    Plot the number of significantly upregulated and downregulated genes per perturbation.

    Parameters:
    - results_dict: Dictionary of DESeq2 results.
    - p_value_threshold: p-value threshold for DEGs.
    - l2fc_threshold: log2 fold change threshold for DEGs.
    - deg_cutoff: Minimum number of DEGs to highlight.
    """
