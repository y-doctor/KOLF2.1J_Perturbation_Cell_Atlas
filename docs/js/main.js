...
const MDE = (() => {
  ...
  function ensureFitness() { return fitnessCache || (fitnessCache = fetch('data/mde/fitness.json?v=30').then(r => r.json())); }
  ...
  function init() {
    return fetch('data/mde.json?v=30').then(r => r.json()).then(payload => {
      data = payload.points;
...
const Clustermap = makeClustermap({
  ...
  metaPath: 'data/clustermap/meta.json?v=30',
  mainPath: 'data/clustermap/corr_int8.bin?v=30',
  sidePath: 'data/clustermap/corr_side_int8.bin?v=30',
...
const GeneClustermap = makeClustermap({
  ...
  metaPath: 'data/genemap/meta.json?v=30',
  mainPath: 'data/genemap/gene_corr_int8.bin?v=30',
  sidePath: 'data/genemap/gene_corr_side_int8.bin?v=30',
  mainZmin: -0.2, mainZmax: 0.2,
  sideZmin: -0.5, sideZmax: 0.5,
});
...
const GeneQuery = (() => {
  ...
  const MODES = {
    gene: {
      indexPath: 'data/genes/index.json?v=30',
      topkPath:  'data/genes/topk.json?v=30',
    ...
    pert: {
      indexPath: 'data/perts/index.json?v=30',
      topkPath:  'data/perts/topk.json?v=30',
      indexKey:  'perts',
      counterKey:'n_genes',
...
const Clusters = (() => {
  ...
  function ensureData() {
    ...
    loaded = fetch('data/clusters/summary.json?v=30').then(r => r.json()).then(d => {
      data = d;
...
const Panel = (() => {
  ...
  function ensurePertMeta() {
    ...
    pmLoaded = fetch('data/panel/all/meta.json?v=30').then(r => r.json()).then(m => {
      pmMeta = m;
  ...
  function fetchPertRow(name) {
    ...
    const url = `data/panel/all/chunk_${chunkIdx}.bin?v=30`;
  ...
  function ensureClusterMeans() {
    ...
    clLoaded = Promise.all([
      fetch('data/clusters/means_meta.json?v=30').then(r => r.json()),
      fetch('data/clusters/means.bin?v=30').then(r => r.arrayBuffer()),
    ]).then(([m, buf]) => {
      clMeta = m;
  ...
  function ensureClusterSummary() {
    ...
    clSummaryLoaded = fetch('data/clusters/summary.json?v=30').then(r => r.json()).then(s => {
      clSummary = s;
